       ...
       (function($) {
            ...
            d3.select(self.frameElement).style("height", "' . $this->height . '");
                ...
                function click(d) {
				  ...
				  if (d.children == null && d._children == null && url != "")
				  {																									
					console.log(d.name);
					$.get(url+"/depth/"+d.depth+"/name/"+d.name,
						function(json)
						{	
							//console.log(json);
							json = $.parseJSON(json);
							...
							if (json.status)
							{
								//d.children = [{"name":"tes","children":[]}];
								d.children = json.data;
								update(root);	
								jml = jml+d.children.length;
								var nh = jml*40;
								//$("#collapsibleTree	svg g,#collapsibleTree	svg").height(nh);
								height = nh;
