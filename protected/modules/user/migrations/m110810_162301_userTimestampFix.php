	...
	public function safeUp()
	{
        ...
        switch ($this->dbType()) {
            ...
            case "pgsql":
                    ...
                    $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET create_at = to_timestamp(createtime), lastvisit_at = (CASE WHEN lastvisit > 4713 THEN to_timestamp(lastvisit) ELSE '1970-01-01 00:00:00' END)");
                    $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'createtime');
                    $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit');
                break;    
            case "sqlite":
	...
	public function safeDown()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                ...
                break;
            case "pgsql":
                $this->addColumn(Yii::app()->getModule('user')->tableUsers,'createtime',"int4 NOT NULL");
                $this->addColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit',"int4 NOT NULL");
                $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET createtime = EXTRACT(EPOCH FROM TIMESTAMP create_at), lastvisit = EXTRACT(EPOCH FROM TIMESTAMP lastvisit_at)");
                $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'create_at');
                $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit_at');
                break;    
            case "sqlite":
