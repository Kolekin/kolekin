...
class ProfileFieldController extends Controller
{
	...
	public $layout = '//layouts/dashboard/dashboard';
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['ProfileField']))
		{
			...
			if($model->validate()) {
				...
				if (substr($model->getDbConnection()->connectionString,0,5) == 'pgsql') {
					$sql = 'ALTER TABLE '.Profile::model()->tableName().' ADD '.$model->varname.' ';
					if ($model->field_type=='INTEGER' && intval($model->field_size)<=4) {
						$sql .= 'INT2';	
					}	
					elseif ($model->field_type=='INTEGER' && intval($model->field_size)>4 && intval($model->field_size)<=9) {
						$sql .= 'INT4';	
					}	
					elseif ($model->field_type=='INTEGER' && intval($model->field_size)>9 && intval($model->field_size)<=18) {
						$sql .= 'INT8';	
					}	
					elseif ($model->field_type=='INTEGER' && intval($model->field_size)>18) {
						$sql .= 'NUMERIC';	
					}	
					else {
					...
					if ($model->default) {
						...
						if (substr($model->getDbConnection()->connectionString,0,5) == 'pgsql') {
							$sql .= ((
										...
										||$model->field_type=='BLOB'
										||$model->field_type=='BINARY'
									)?" DEFAULT ''":(($model->field_type=='DATE')?" DEFAULT '1970-01-01'":" DEFAULT 0"));
						}
						else {
							$sql .= ((
										$model->field_type=='TEXT'
